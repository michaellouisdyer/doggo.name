...
def train(model, train_generator, validation_datagen, train_folder, validation_folder, epochs, batch_size, CPUS = 4):
    """
    Trains the model
    """
    ...
    tensorboard = keras.callbacks.TensorBoard(log_dir=logdir, histogram_freq=0, batch_size=batch_size, write_graph=True, embeddings_freq=0)

    # Create a callback for each model checkpoint
    mc =  keras.callbacks.ModelCheckpoint('./models/'+project_name+'.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
def create_learn_rate_dict(model):
    ...
    layer_mult = dict(zip([layer.name for layer in model.layers], itertools.repeat(base_layer_learn_ratio)))
...
if __name__ == '__main__':
    ...
    train_datagen, validation_datagen,train_generator, validation_generator = create_generators(augmentation_strength, target_size, batch_size, train_folder, validation_folder)

    train(model, train_datagen, validation_datagen, train_generator, validation_generator, epochs, batch_size, CPUS)
